 ...
 * 字母导航控件。
 */
class DoraLetterView @JvmOverloads constructor(
    context: Context,
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        val a = context.obtainStyledAttributes(attrs, R.styleable.DoraLetterView, defStyleAttr, 0)
        letters = parseLetters(a.getString(R.styleable.DoraLetterView_dview_lv_letters))
        letterTextSize = a.getDimension(
            R.styleable.DoraLetterView_dview_lv_textSize,
            TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 15f, metrics)
        )
        letterTextColor = a.getColor(R.styleable.DoraLetterView_dview_lv_textColor, DEFAULT_TEXT_COLOR)
        letterHoverTextColor = a
            .getColor(R.styleable.DoraLetterView_dview_lv_hoverTextColor, DEFAULT_HOVER_TEXT_COLOR)
        hoverBackgroundDrawable =
            a.getDrawable(R.styleable.DoraLetterView_dview_lv_hoverBackgroundDrawable)
        textAllCaps = a.getBoolean(R.styleable.DoraLetterView_dview_lv_textAllCaps, false)
        a.recycle()
    }
